        ...
        switch ($_GET['params'][0]) {
            ...
            case 'solve':
                ...
                if(!userLevel($data['view_level']))
                    abort(400);
                $sql = 'select distinct id,problem,answer,wrong from problems where qualification_id = '.$_GET['params'][1].' and subject_id = '.$_GET['params'][2].' order by rand()';
    ...
    case 'POST':
        switch ($_POST['mode']) {
            ...
            case 'solve':
                ...
                $res = [
                    'correct' => $correct,
                    'c' => $data['w_cnt'] + $data['c_cnt'] == 0 ? '100' : ($data['c_cnt'] / ($data['w_cnt'] + $data['c_cnt'])) * 100
                ];
                if(!$correct)
                    $res['answer'] = $answer;
                apiEnd($res);
